...
public class MortarG : MonoBehaviour
{
    ...
    void FindClosestEnemy()
    {
        ...
        else
        {
            ...
            if (reloadtime <= 0)
            {
                target.position = new Vector3(closestEnemy.transform.position.x + Random.Range(-spread, spread), closestEnemy.transform.position.y + Random.Range(-spread, spread), closestEnemy.transform.position.z + Random.Range(-spread, spread));
                ...
                for (int i = 0; i < enemiesToDamage.Length; i++)
                {
                    ...
                    {
                        enemiesToDamage[i].GetComponent<Enemy>().TakeDamage(dmg + 50);
                    }
                    else
                    {
